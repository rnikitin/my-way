...
**How to apply.** Promote repeated mistakes into automation: link checks for docs, schema checks for structured files, type checks for shared contracts, visual smoke tests for UI, secret scanners for public repos, and approval gates for destructive commands. Run a harness audit such as [`harness-audit`](harness-audit.md) when the agent-facing surface changes.
